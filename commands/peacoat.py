...
def get_char(name, server, target_region):
    r = requests.get("https://%s.api.battle.net/wow/character/%s/%s?fields=items+progression+achievements+statistics&locale=%s&apikey=%s" % (
    ...
    max_ilvl = player_dict["items"]["averageItemLevel"]
    ...
    # iLvL
    return_string += "Max iLevel: %s\n" % max_ilvl
    return_string += "Actual iLevel: %s\n" % equipped_ivl
...
async def pug(client, message):
    ...
    try:
        ...
        print(e)
        await client.send_message(message.channel, "Looks like you are bad.\n"
